// Pure render: no DOM, usable on server (SSR/DSD) and client. The client
// connectedCallback calls this then wires events; the server emits DSD from it.
export function buttonTemplate(attrs = {}) {
  const html = `<button part="button" type="button">
         <span class="spin" part="spinner" aria-hidden="true"></span>
         <span class="label"><slot></slot></span>
       </button>`;
  return { html, css: BUTTON_CSS };
}
