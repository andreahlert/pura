// Pure render for <pura-badge>. No DOM; safe on server (SSR/DSD) and client.
import { EMPTY_SHIM } from "../base.js";

export function badgeTemplate(el = EMPTY_SHIM) {
  const html = `<span part="badge">${el.hasAttribute("dot") ? '<span class="dot" aria-hidden="true"></span>' : ""}<slot></slot></span>`;
  return { html, css: BADGE_CSS };
}
